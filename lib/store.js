...
function checkLicense(input, req) {
  ...
  let user = db.users.find((item) => String(item.account) === account);
  if (!user && !phone) {
  ...
  const storedBroker = String(user.broker || "").trim().toLowerCase();
  ...
  if (!license && phone) {
    if (hasTrialHistory(db, account, robot.id)) {
      return saveCheckIn(db, {
        ...base,
  ...
  if (new Date(license.expiresAt) <= now) {
    const trialExpired = String(license.type || "").toUpperCase() === "TRIAL";
    return saveCheckIn(db, {
      ...base,
  ...
  return saveCheckIn(db, {
    ...
    robotId: robot.id,
    licenseId: license.id,
    message: String(license.type || "").toUpperCase() === "TRIAL"
...
function applyApprovedPayment(payment, providerPayload = {}) {
  ...
  let user = db.users.find((item) => String(item.account) === String(payment.account));
  if (!user) {
    user = createUser({
...
function hasTrialHistory(db, account, robotId) {
  db.trialHistory = Array.isArray(db.trialHistory) ? db.trialHistory : [];
  return db.trialHistory.some((item) => item.account === String(account) && item.robotId === robotId);
}

function registerTrialHistory(db, { account, user, robot, license, phone, req }) {
  db.trialHistory = Array.isArray(db.trialHistory) ? db.trialHistory : [];
  if (hasTrialHistory(db, account, robot.id)) return;
