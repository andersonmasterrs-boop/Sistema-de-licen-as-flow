...
function checkLicense(input, req) {
  ...
  if (!user && !phone) {
    return savePendingRequest(db, saveCheckIn(db, {
      ...
      robotId: robot.id,
      message: "Informe seu telefone/WhatsApp nos parametros do robo para liberar 7 dias de teste automatico."
    }, req));
  }
  ...
  if (!user && phone) {
    user = createUser({
      ...
      type: "Real",
      notes: `Criado automaticamente pelo teste de 7 dias. Telefone/WhatsApp: ${phone}.`
    });
  ...
  if (new Date(license.expiresAt) <= now) {
    ...
    return saveCheckIn(db, {
      ...
      message: trialExpired
        ? "Seu periodo de teste de 7 dias acabou. Para continuar usando, efetue a compra da licenca."
  ...
  return saveCheckIn(db, {
    ...
    message: String(license.type || "").toUpperCase() === "TRIAL"
      ? `Teste automatico liberado ate ${new Date(license.expiresAt).toLocaleDateString("pt-BR")}.`
      : (robot.message || "")
  }, req);
}
