...
async function loadDb() {
  ...
  try {
    if (hasSupabaseStore()) {
      ...
      if (process.env.LICENSE_ALLOW_DB_INITIALIZATION === "true") {
        const saved = await saveDb();
        if (!saved) throw new Error("PERSISTENCE_INITIALIZATION_FAILED");
        return getDb();
      }

      throw new Error("PERSISTENCE_STATE_NOT_FOUND");
    }
    ...
    // Never continue with a stale serverless instance after a persistent read
    // failure: a later save could replace the complete database with this copy.
    throw error;
  }
}
...
function buildState() {
  ...
  return {
    ...
    performanceAttempts: (db.performanceAttempts || []).slice(0, 120),
...
function reportPerformance(input, req) {
  ...
  const robot = db.robots.find((item) => item.name.toLowerCase() === robotName.toLowerCase());
  if (!robot) return savePerformanceAttempt(db, input, { ok: false, error: "ROBOT_NOT_FOUND" }, req);

  let user = findUserByAccount(db, account);
  if (!user && accountName) user = findUserByAccountHolder(db, accountName);
  if (!user && phone) user = findPaidUnlinkedUserByPhone(db, phone, robot.id);
  if (!user) return savePerformanceAttempt(db, input, { ok: false, error: "ACCOUNT_NOT_FOUND", robotId: robot.id }, req);
  if (user.status && user.status !== "active") return savePerformanceAttempt(db, input, { ok: false, error: "USER_INACTIVE", userId: user.id, robotId: robot.id }, req);

  const license = db.licenses.find((item) => item.userId === user.id && item.robotId === robot.id);
  if (!license) return savePerformanceAttempt(db, input, { ok: false, error: "LICENSE_NOT_FOUND", userId: user.id, robotId: robot.id }, req);
  if (license.status !== "active") return savePerformanceAttempt(db, input, { ok: false, error: "LICENSE_INACTIVE", userId: user.id, robotId: robot.id, licenseId: license.id }, req);
  if (new Date(license.expiresAt) <= now) return savePerformanceAttempt(db, input, { ok: false, error: "LICENSE_EXPIRED", userId: user.id, robotId: robot.id, licenseId: license.id }, req);
  const linkedAccount = addAccountToUser(user, { account, name: accountName, broker: brokerRaw, accountServer, updateName: true });
  ...
  if (existing) {
    ...
    savePerformanceAttempt(db, input, { ok: true, userId: user.id, robotId: robot.id, licenseId: license.id }, req);
...
function savePerformanceAttempt(db, input, result, req) {
  db.performanceAttempts = Array.isArray(db.performanceAttempts) ? db.performanceAttempts : [];
  db.performanceAttempts.unshift({
    id: createId("pat"),
    at: new Date().toISOString(),
    ok: Boolean(result.ok),
    error: result.error || "",
    account: String(input.account || ""),
    accountName: String(input.name || input.accountName || ""),
    robot: String(input.robot || ""),
  ...
  db.performanceAttempts = db.performanceAttempts.slice(0, 300);
  return result;
}
