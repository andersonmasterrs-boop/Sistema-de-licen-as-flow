...
function checkLicense(input, req) {
  ...
  return saveCheckIn(db, {
    ...
    message: licenseType(license) === "TRIAL"
      ...
      : (takeVersionMessage(robot, license, account, clientVersion) || takeRobotMessage(robot, license, account))
  }, req);
}
...
function applyApprovedPayment(payment, providerPayload = {}) {
  ...
  let user = findUserByAccount(db, payment.account) || findUserByAccountHolder(db, payment.name);
  if (!user) {
    user = createUser({
      account: payment.account,
      name: payment.name || `Conta ${payment.account}`,
...
function takeVersionMessage(robot, license, account, clientVersion) {
  const requiredVersion = String(robot.version || "").trim();
  const currentVersion = String(clientVersion || "").trim();
  if (!requiredVersion || !currentVersion || compareVersions(currentVersion, requiredVersion) >= 0) return "";

  const messageId = `version-${requiredVersion}`;
  const target = String(account || license.id || "").trim();
  license.deliveredMessages = Array.isArray(license.deliveredMessages) ? license.deliveredMessages : [];
  if (license.deliveredMessages.some((item) => item.messageId === messageId && String(item.account || "") === target)) return "";
  license.deliveredMessages.unshift({ messageId, account: target, deliveredAt: new Date().toISOString() });
  license.deliveredMessages = license.deliveredMessages.slice(0, 50);
  license.updatedAt = new Date().toISOString();
  return `Existe uma nova versao do ${robot.name}: ${requiredVersion}. Atualize seu robo quando possivel.`;
}
