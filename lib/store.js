...
function checkLicense(input, req) {
  ...
  if (!user && phone) user = findUserByPhone(db, phone);
  if (!user && !phone) {
  ...
  const linkedAccount = addAccountToUser(user, { account, name: accountName, broker: brokerRaw, accountServer });
...
function addAccountToUser(user, accountData) {
  ...
  if (entry) {
    if (accountData.name && !entry.name) entry.name = String(accountData.name).trim();
...
function applyApprovedPayment(payment, providerPayload = {}) {
  ...
  let user = findUserByAccount(db, payment.account) || findUserByPhone(db, payment.phone);
  if (!user) {
    user = createUser({
